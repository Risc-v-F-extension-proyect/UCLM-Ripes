...
void FPUnicicleDiagramWidget::drawDiagram() {
  ...
  const std::array<Unit, 4> units{{
      ...
      {"FP add/sub",
       std::clamp(number(RIPES_SETTING_RV5S_FALU_ADDSUB_LATENCY), 2u, 7u),
       addSubSegmented
           ? 1u
      ...
      {"FP multiply",
       std::clamp(number(RIPES_SETTING_RV5S_FALU_MUL_LATENCY), 2u, 12u),
       multiplySegmented
           ? 1u
           : std::clamp(number(RIPES_SETTING_RV5S_FALU_MUL_COUNT), 1u, 4u),
       multiplySegmented, QColor(95, 211, 218)},
      {"FP divide",
       std::clamp(number(RIPES_SETTING_RV5S_FALU_DIV_LATENCY), 2u, 30u),
       divideSegmented
           ? 1u
           : std::clamp(number(RIPES_SETTING_RV5S_FALU_DIV_COUNT), 1u, 4u),
  ...
  constexpr qreal routingBarW = 38;
  const qreal routingBarX = idX + pipelineW + 28;
  const qreal outputBarX = unitEndX + 32;
  const qreal memX = outputBarX + routingBarW + 40;
  ...
  const qreal barY = layouts.front().y - 12;
  const qreal barH = layouts.back().y + cellH + 12 - barY;
  const qreal routingBarRight = routingBarX + routingBarW;
  const qreal outputBarRight = outputBarX + routingBarW;
  const qreal inputSplitX = routingBarX - 22;
  line(ifX + pipelineW, axis, idX, axis);
  line(idX + pipelineW, axis, inputSplitX, axis);
  for (const auto &layout : layouts) {
    const qreal centerY = layout.y + cellH / 2;
    line(inputSplitX, axis, routingBarX, centerY);
    line(routingBarRight, centerY, layout.firstX, centerY);

    const qreal lastRight = layout.lastX + cellW;
    line(lastRight, centerY, outputBarX, centerY);
  }
  line(outputBarRight, axis, memX, axis);
  ...
  const auto verticalRoutingBar = [this, cardPen](const QString &label,
                                                   qreal x, qreal y,
                                                   qreal width, qreal height,
                                                   const QColor &fill) {
    QPainterPath path;
    path.addRoundedRect(QRectF(x, y, width, height), 5, 5);
    m_scene->addPath(path, cardPen, QBrush(fill));

    if (label.isEmpty())
      return;
    auto *item = m_scene->addText(label);
    QFont font = item->font();
    font.setBold(true);
    item->setFont(font);
    item->setDefaultTextColor(Qt::black);
    ...
    item->setPos(x + (width - bounds.width() * item->scale()) / 2,
                 y + (height - bounds.height() * item->scale()) / 2);
  };

  const std::array<QString, 4> inputPipelineNames{
      "ID/\nEX", "ID/\nA1", "ID/\nM1", "ID/\nD1"};
  for (const auto &layout : layouts) {
    verticalRoutingBar(inputPipelineNames.at(layout.type), routingBarX,
                       layout.y, routingBarW, cellH, QColor(220, 232, 246));
  }
  verticalRoutingBar({}, outputBarX, barY, routingBarW, barH,
                     QColor(232, 235, 240));

  constexpr qreal legendX = 20;
  constexpr qreal legendY = 18;
  constexpr qreal legendW = 270;
  ...
  for (const auto &layout : layouts) {
    ...
    for (unsigned stage = 0; stage < unit.latency; ++stage) {
      ...
      } else if (type == 0 && stage == 0 &&
                 pipelineInfo.at(2).stage_valid && !fpInClassicEX) {
...
void FPUnicicleDiagramWidget::fitDiagram() {
  ...
  const qreal scale = std::min<qreal>(
      1.15, std::max<qreal>(1.0, m_view->viewport()->height() - 8) /
                rect.height());
