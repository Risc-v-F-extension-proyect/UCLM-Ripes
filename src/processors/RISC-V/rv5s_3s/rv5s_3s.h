...
public:
  ...
  RV5S_3S(const QStringList &extensions)
    ...
    fee_enable_or->out >> instruction_tag_generator->enable_in;
    ...
    fe_window_enable_and->out >> *fee_enable_or->in[1];
    ...
    exmem_reg->do_branch_out >> *controlflow_not->in[0];
    idex_reg->valid_out >> *falu_enable_and->in[0];
    controlflow_not->out >> *falu_enable_and->in[1];
    falu_enable_and->out >> falu->enable_in;
    0 >> falu->bypass_in;
    ...
    controlflow_or->out >> pending_instruction_window->do_branch_in;	// MODIFIED
    controlflow_or->out >> pending_instruction_window->controlflow_flush_in; // MODIFIED
    ...
    pending_instruction_window->do_branch_out >> exmem_reg->do_branch_in;	// MODIFIED
  ...
  SUBCOMPONENT(controlflow_not, TYPE(Not<1, 1>));
  SUBCOMPONENT(falu_enable_and, TYPE(And<1, 2>));
