...
#include "rv5s_2s_db_pcpersistence.h"
...
class RV5S2SDBAddressAdapter : public Component {
public:
  RV5S2SDBAddressAdapter(const std::string &name, SimComponent *parent)
...
public:
  ...
  RV5S_2S_DB(const QStringList &extensions)
      : RipesVSRTLProcessor("5-Stage RISC-V Processor (2-slot delayed branch)") {
    ...
    pcPersistence->setDelayedSlotSources(
        ...
        [this] { return idex_reg->pc_out.uValue(); });
    ...
    controlflow_or->out >> pcPersistence->branchTaken;
    alu->res >> pcPersistence->branchTarget;
  ...
  SUBCOMPONENT(pcPersistence, TYPE(RV5S2SDBPCPersistence<XLEN>));
  ...
  SUBCOMPONENT(hazard_pc_adapter, TYPE(RV5S2SDBAddressAdapter<XLEN>));
