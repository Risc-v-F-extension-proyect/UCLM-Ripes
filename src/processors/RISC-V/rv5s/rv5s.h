...
public:
  ...
  std::vector<FPUnicicleStageInfo> fpUnicicleStageInfos() const override {
    ...
    for (const auto &unit : hzunit->functionalUnits()) {
      if (unit.type == HazardUnit::FUType::Integer) {
        continue;
      }
  ...
  AdvancedExecutionStatistics advancedExecutionStatistics() const override {
    ...
    statistics.dataHazardStallHistory = m_dataHazardStallHistory;
    statistics.structuralHazardStallHistory =
        m_structuralHazardStallHistory;
    statistics.controlHazardStallHistory = m_controlHazardStallHistory;
    statistics.cycleHistory = m_advancedStatisticsCycleHistory;
  ...
  void reverse() override {
    ...
    if (!m_dataHazardStallHistory.empty())
      m_dataHazardStallHistory.pop_back();
    if (!m_structuralHazardStallHistory.empty())
      m_structuralHazardStallHistory.pop_back();
    if (!m_controlHazardStallHistory.empty())
      m_controlHazardStallHistory.pop_back();
    if (!m_advancedStatisticsCycleHistory.empty())
      m_advancedStatisticsCycleHistory.pop_back();
    if (memwb_reg->valid_out.uValue() != 0 &&
        isExecutableAddress(memwb_reg->pc_out.uValue())) {
      m_instructionsRetired--;
    }
  ...
  void reset() override {
    ...
    m_dataHazardStallHistory.clear();
    m_structuralHazardStallHistory.clear();
    m_controlHazardStallHistory.clear();
    m_advancedStatisticsCycleHistory.clear();
  }
...
private:
  void saveAdvancedStatistics() {
    ...
    const bool structuralHazard = hzunit->structuralHazardActive();
    if (dataHazard)
    ...
    m_dataHazardStallHistory.push_back(
        m_advancedStatistics.dataHazardStallCycles);
    m_structuralHazardStallHistory.push_back(
        m_advancedStatistics.structuralHazardStallCycles);
    m_controlHazardStallHistory.push_back(
        m_advancedStatistics.controlHazardStallCycles);
    m_advancedStatisticsCycleHistory.push_back({
        m_advancedStatistics.instructionMemoryCycles,
        m_advancedStatistics.dataMemoryCycles,
        m_advancedStatistics.aluCycles,
        m_advancedStatistics.integerUnitCycles,
        m_advancedStatistics.fpAddSubUnitCyclesByInstance,
        m_advancedStatistics.fpMultiplyUnitCyclesByInstance,
        m_advancedStatistics.fpDivideUnitCyclesByInstance,
        m_advancedStatistics.stallCycles,
        m_advancedStatistics.dataHazardStallCycles,
        m_advancedStatistics.structuralHazardStallCycles,
        m_advancedStatistics.controlHazardStallCycles});
  ...
  std::vector<uint64_t> m_dataHazardStallHistory;
  std::vector<uint64_t> m_structuralHazardStallHistory;
  std::vector<uint64_t> m_controlHazardStallHistory;
  std::vector<AdvancedExecutionStatisticsSample>
      m_advancedStatisticsCycleHistory;
