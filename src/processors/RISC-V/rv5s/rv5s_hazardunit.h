...
public:
  ...
  void setConfiguration(unsigned addLatency, unsigned multiplyLatency,
    ...
    addSubLatency = std::clamp(addLatency, 2u, 7u);
    mulLatency = std::clamp(multiplyLatency, 2u, 12u);
    divLatency = std::clamp(divideLatency, 2u, 30u);
    addSubCount = addPipelined ? 1u : std::clamp(addCount, 1u, 4u);
    mulCount = multiplyPipelined ? 1u : std::clamp(multiplyCount, 1u, 4u);
    divCount = dividePipelined ? 1u : std::clamp(divideCount, 1u, 4u);
  ...
  void prioritizeControlFlowAt(uint64_t cycle) {
    ...
    for (const auto &unit : functionalUnitList) {
      for (const auto &instruction : unit.stages) {
        if (instruction.valid && isControlFlow(instruction.opcode) &&
            instruction.emissionCycle <= cycle) {
          priorityInstructionId = instruction.id;
          break;
        }
      }
      if (priorityInstructionId != 0)
        break;
    }

    std::set<uint64_t> occupiedCycles;

    // El salto conserva cycle. Solo se desplaza el bloque contiguo de
    // reservas que comienza en ese ciclo; una reserva situada tras el primer
    // hueco no resulta afectada.
    for (const auto &unit : functionalUnitList) {
      for (const auto &instruction : unit.stages) {
        if (instruction.valid && instruction.id != priorityInstructionId &&
            instruction.emissionCycle >= cycle)
...
public:
  ...
  void setExternalAcceptanceCondition(std::function<bool()> condition) {
