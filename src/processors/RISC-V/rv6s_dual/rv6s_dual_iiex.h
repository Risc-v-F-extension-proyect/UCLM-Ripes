...
public:
  RV5S_IIEX_DUAL(const std::string &name, SimComponent *parent)
      : RV5S_IDEX<XLEN>(name, parent) {
    ...
    CONNECT_REGISTERED_CLEN_INPUT(reg_wr_src_ctrl_dual, this->clear,
                                  this->enable);
    CONNECT_REGISTERED_CLEN_INPUT(reg_wr_src_ctrl_data, this->clear,
                                  this->enable);
