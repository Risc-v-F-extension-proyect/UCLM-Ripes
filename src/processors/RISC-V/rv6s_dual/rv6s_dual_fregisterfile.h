...
class FRegisterFile_DUAL : public Component {
public:
  SetGraphicsType(ClockedComponent);
  FRegisterFile_DUAL(const std::string &name, SimComponent *parent)
  ...
  SUBCOMPONENT(rf_1, TYPE(FRegisterFile<XLEN, readBypass>));
  SUBCOMPONENT(rf_2, TYPE(FRegisterFile<XLEN, readBypass>));
...
private:
  VSRTL_VT_U doReadBypass(const VSRTL_VT_U reg_idx, const int portIndex,
                          const FRegisterFile<XLEN, readBypass> *rf) {
