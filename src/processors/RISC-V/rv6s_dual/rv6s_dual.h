...
public:
  ...
  RV6S_DUAL(const QStringList &extensions)
    ...
    exmem_reg->alures_out >>
        exec_reg1_fw_src->get(ForwardingSrcDual::MemStageExec);
    exmem_reg->alures_data_out >>
        exec_reg1_fw_src->get(ForwardingSrcDual::MemStageMem);
    reg_wr_src->out >> exec_reg1_fw_src->get(ForwardingSrcDual::WbStageExec);
    reg_wr_src_data->out >>
        exec_reg1_fw_src->get(ForwardingSrcDual::WbStageMem);
    funit->alu_reg1_fw_ctrl_exec >> exec_reg1_fw_src->select;

    iiex_reg->r2_out >> exec_reg2_fw_src->get(ForwardingSrcDual::IdStage);
    exmem_reg->alures_out >>
        exec_reg2_fw_src->get(ForwardingSrcDual::MemStageExec);
    exmem_reg->alures_data_out >>
        exec_reg2_fw_src->get(ForwardingSrcDual::MemStageMem);
    reg_wr_src->out >> exec_reg2_fw_src->get(ForwardingSrcDual::WbStageExec);
    reg_wr_src_data->out >>
        exec_reg2_fw_src->get(ForwardingSrcDual::WbStageMem);
    ...
    exmem_reg->alures_out >>
        data_reg1_fw_src->get(ForwardingSrcDual::MemStageExec);
    exmem_reg->alures_data_out >>
        data_reg1_fw_src->get(ForwardingSrcDual::MemStageMem);
    reg_wr_src->out >> data_reg1_fw_src->get(ForwardingSrcDual::WbStageExec);
    reg_wr_src_data->out >>
        data_reg1_fw_src->get(ForwardingSrcDual::WbStageMem);
    funit->alu_reg1_fw_ctrl_data >> data_reg1_fw_src->select;

    iiex_reg->r2_data_out >> data_reg2_fw_src->get(ForwardingSrcDual::IdStage);
    exmem_reg->alures_out >>
        data_reg2_fw_src->get(ForwardingSrcDual::MemStageExec);
    exmem_reg->alures_data_out >>
        data_reg2_fw_src->get(ForwardingSrcDual::MemStageMem);
    reg_wr_src->out >> data_reg2_fw_src->get(ForwardingSrcDual::WbStageExec);
    reg_wr_src_data->out >>
        data_reg2_fw_src->get(ForwardingSrcDual::WbStageMem);
    ...
    0 >> iiex_reg->reg_wr_src_ctrl_in; // unused - we're using the specialized
                                       // RegWrSrcDual
    ...
    // Data
    iiex_reg->pc_out >>
        exmem_reg->pc_in; // @todo: Fix this - needs multiplexer aswell
    iiex_reg->pc_data_out >>
        exmem_reg->pc_data_in; //@todo: Fix this - needs multiplexer aswell
    ...
    memwb_reg->alures_data_out >>
        reg_wr_src_data->get(RegWrSrcDataDual::ALURES);
  ...
  SUBCOMPONENT(exec_reg1_fw_src,
               TYPE(EnumMultiplexer<ForwardingSrcDual, XLEN>));
  SUBCOMPONENT(exec_reg2_fw_src,
               TYPE(EnumMultiplexer<ForwardingSrcDual, XLEN>));

  SUBCOMPONENT(data_reg1_fw_src,
               TYPE(EnumMultiplexer<ForwardingSrcDual, XLEN>));
  ...
  SUBCOMPONENT(data_way_r1_reg_idx,
               TYPE(EnumMultiplexer<WaySrc, c_RVRegsBits>));
  SUBCOMPONENT(data_way_r2_reg_idx,
               TYPE(EnumMultiplexer<WaySrc, c_RVRegsBits>));
  ...
  SUBCOMPONENT(exec_way_wr_reg_idx,
               TYPE(EnumMultiplexer<WaySrc, c_RVRegsBits>));
  SUBCOMPONENT(exec_way_r1_reg_idx,
               TYPE(EnumMultiplexer<WaySrc, c_RVRegsBits>));
  SUBCOMPONENT(exec_way_r2_reg_idx,
               TYPE(EnumMultiplexer<WaySrc, c_RVRegsBits>));
  ...
  int instructionsRetired() const {
    ...
    if (memwb_reg->valid_out.uValue() != 0) {
      if (isExecutableAddress(memwb_reg->pc_out.uValue()) &&
          memwb_reg->exec_valid_out.uValue()) {
        amount++;
      }
      if (isExecutableAddress(memwb_reg->pc4_out.uValue()) &&
          memwb_reg->data_valid_out.uValue()) {
        amount++;
      }
    }
