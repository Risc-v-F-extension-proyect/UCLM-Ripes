...
AInt InstructionModel::indexToAddress(const QModelIndex &index) const {
  if (m_program) {
    ...
    if (disassembleRes.numInstructions() < static_cast<unsigned>(index.row()))
      return 0;
    if (auto addr = disassembleRes.indexToAddress(index.row());
        addr.has_value())
      return addr.value();
  }
  return 0;
...
InstructionModel::InstructionModel(QObject *parent)
  ...
  connect(ProcessorHandler::get(), &ProcessorHandler::processorReset, this,
          &InstructionModel::onProcessorReset);
  onProcessorReset();
}
...
int InstructionModel::rowCount(const QModelIndex &) const { return m_rowCount; }
...
bool InstructionModel::setData(const QModelIndex &index, const QVariant &value,
  ...
  if ((index.column() == Column::Breakpoint) && role == Qt::CheckStateRole) {
    if (static_cast<Qt::CheckState>(value.toInt()) == Qt::Checked) {
      ProcessorHandler::setBreakpoint(addr,
                                      !ProcessorHandler::hasBreakpoint(addr));
      return true;
    }
  }
  return false;
...
QVariant InstructionModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const {
  if (orientation == Qt::Horizontal &&
    ...
    case Column::Stage:
      return role == Qt::DisplayRole ? "Stage"
                                     : "Stages currently executing instructon";
